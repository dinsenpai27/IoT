...
void kontrolPompa(float suhu, int kelembapanTanah) {
  if (suhu >= 15 && suhu < 20) {
    if (kelembapanTanah <= 20) { // Kelembapan tanah rendah (lebih kering)
      ...
      Serial.println("Pompa air AKTIF (Suhu 15-20°C, Kelembapan <= 20%)");
    } else {
      digitalWrite(RELAY_PIN, LOW); // Pompa mati (tanah lembab)
      Serial.println("Pompa air MATI (Suhu 15-20°C, Kelembapan > 20%)");
    }
  } else if (suhu >= 20 && suhu < 25) {
    if (kelembapanTanah <= 20) { // Kelembapan tanah rendah (lebih kering)
      digitalWrite(RELAY_PIN, HIGH);  // Pompa aktif (tanah kering)
      Serial.println("Pompa air AKTIF (Suhu 20-25°C, Kelembapan <= 20%)");
    } else {
      digitalWrite(RELAY_PIN, LOW); // Pompa mati (tanah lembab)
      Serial.println("Pompa air MATI (Suhu 20-25°C, Kelembapan > 20%)");
    }
  } else if (suhu >= 25 && suhu < 30) {
    if (kelembapanTanah <= 20) { // Kelembapan tanah rendah (lebih kering)
      digitalWrite(RELAY_PIN, HIGH);  // Pompa aktif (tanah kering)
      Serial.println("Pompa air AKTIF (Suhu 25-30°C, Kelembapan <= 20%)");
    } else {
      digitalWrite(RELAY_PIN, LOW); // Pompa mati (tanah lembab)
      Serial.println("Pompa air MATI (Suhu 25-30°C, Kelembapan > 20%)");
    }
  } else if (suhu >= 30 && suhu < 35) {
    if (kelembapanTanah <= 20) { // Kelembapan tanah rendah (lebih kering)
      digitalWrite(RELAY_PIN, HIGH);  // Pompa aktif (tanah kering)
      Serial.println("Pompa air AKTIF (Suhu 30-35°C, Kelembapan <= 20%)");
    } else {
      digitalWrite(RELAY_PIN, LOW); // Pompa mati (tanah lembab)
      Serial.println("Pompa air MATI (Suhu 30-35°C, Kelembapan > 20%)");
    }
  }
}
...
void loop() {
  ...
  // Tentukan status pompa dan kirimkan data status pompa
